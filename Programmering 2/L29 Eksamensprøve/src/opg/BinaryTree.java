...
public class BinaryTree<E> {
    ...
    private int countElements(Node node, E element) {
        if (node == null)
            return 0;
        else if (node.data.equals(element))
            return countElements(node.left, element) + 1 + countElements(node.right, element);
        else
            return countElements(node.left, element) + countElements(node.right, element);
    }
